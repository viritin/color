 ...
 *
 * Required because compiled enum constants (resolved against a Maven JAR) expose
 * {@code getArgumentList() == null} — the constructor argument lives in the
 * enum's static initializer bytecode, not in PSI. Reading hexes from the
 * library JAR therefore needs an out-of-band table.
 *
 * Insertion order matches the source enum so completion lists match the IDE's
 * native CSS popup ordering.
 */
final class NamedColorTable {
    ...
    static @Nullable Color colorFor(String enumConstantName) {
